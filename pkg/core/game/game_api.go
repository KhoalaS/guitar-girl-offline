...
func (api *GameApi) UserSave(params user_model.UserSave, apiCategory string) BaseGameResponse[user_model.UserSaveRetDataInfo] {
	status, err := api.service.UserSave(params.Data)
	return NewBaseGameResponse(params.Call, apiCategory, api.timeZone, status, err)
}
