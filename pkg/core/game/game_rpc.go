...
func (gameRpc *GameRpc) registerUserMux() {
	...
	mux.HandleFunc("/userLoad/en/", gameRpc.userLoad)
	mux.HandleFunc("/setSubscribe/en/", gameRpc.setSubscribe)
...
func (gameRpc *GameRpc) registerStoreMux() {
	...
	mux.HandleFunc("/buyVarietyStore/en/", gameRpc.buyVarietyStore)
	mux.HandleFunc("/buyCheck/en/", gameRpc.buyCheck)
...
func (gameRpc *GameRpc) buyVarietyStore(w http.ResponseWriter, r *http.Request) {
	requestDto, _ := getRequestDto(r)

	baseRequest, _ := rpc.ThriftDataToAny[store_model.BuyVarietyStore](requestDto.TapsonicData)
	response := gameRpc.api.BuyVarietyStore(baseRequest, "store")
	responseData, _ := thrifter.Marshal(response)

	thriftBytes, _ := rpc.ThriftBytesToBz2B64(responseData)
	w.Write([]byte(thriftBytes))
}

func (gameRpc *GameRpc) buyCheck(w http.ResponseWriter, r *http.Request) {
	requestDto, _ := getRequestDto(r)

	baseRequest, _ := rpc.ThriftDataToAny[store_model.BuyCheck](requestDto.TapsonicData)
	response := gameRpc.api.BuyCheck(baseRequest, "store")
	responseData, _ := thrifter.Marshal(response)
...
func (gameRpc *GameRpc) userLoad(w http.ResponseWriter, r *http.Request) {
	requestDto, _ := getRequestDto(r)

	baseRequest, _ := rpc.ThriftDataToAny[user_model.UserLoad](requestDto.TapsonicData)
	response := gameRpc.api.UserLoad(baseRequest, "user")
	responseData, _ := thrifter.Marshal(response)
...
func (gameRpc *GameRpc) setSubscribe(w http.ResponseWriter, r *http.Request) {
	requestDto, _ := getRequestDto(r)

	baseRequest, _ := rpc.ThriftDataToAny[user_model.SetSubscribe](requestDto.TapsonicData)
	response := gameRpc.api.UserSetSubscribe(baseRequest, "user")
	responseData, _ := thrifter.Marshal(response)
