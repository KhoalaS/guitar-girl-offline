package game

import (
	"net/http"
	"net/url"

	embeds "github.com/KhoalaS/guitar-girl-offline"
	"github.com/KhoalaS/guitar-girl-offline/pkg/rpc"
	"github.com/KhoalaS/thrifter"
	"github.com/rs/zerolog/log"
)

type GameRpc struct {
	api             GameApi
	mainMux         *http.ServeMux
	postMux         *http.ServeMux
	userMux         *http.ServeMux
	storeMux        *http.ServeMux
	rpcHandlers     map[string]RpcHandler
	staticResponses map[string]string
}

type RpcHandler func(gameRpc *GameRpc, category string, tapsonicData string, w http.ResponseWriter, r *http.Request)

func MakeHandler[P any, R any](
	apiFunc func(P, string) BaseGameResponse[R],
) RpcHandler {
	return func(gameRpc *GameRpc, category string, tapsonicData string, w http.ResponseWriter, r *http.Request) {
		baseRequest, _ := rpc.ThriftDataToAny[P](tapsonicData)
		response := apiFunc(baseRequest, category)

		responseData, err := thrifter.Marshal(response)
		if err != nil {
			InternalErrorHandler(w, err)
			log.Debug().Int("code", 3).Err(err).Send()
			return
		}

		thriftBytes, _ := rpc.ThriftBytesToBz2B64(responseData)
		w.Write([]byte(thriftBytes))
	}
}

func (gameRpc *GameRpc) ServeRPC(w http.ResponseWriter, r *http.Request) {
	category := r.PathValue("category")

	requestDto, _ := getRequestDto(r)
	call := requestDto.Call

	if handler, ok := gameRpc.rpcHandlers[call]; ok {
		handler(gameRpc, category, requestDto.TapsonicData, w, r)
		return
	}

	if response, ok := gameRpc.staticResponses[call]; ok {
		w.Write([]byte(response))
		return
	}
	http.NotFound(w, r)
}

func NewGameRpc(api GameApi, mux *http.ServeMux) *GameRpc {
	gameRpc := &GameRpc{
		api:             api,
		rpcHandlers:     make(map[string]RpcHandler),
		staticResponses: make(map[string]string),
	}

	gameRpc.rpcHandlers["buyVarietyStore"] = MakeHandler(gameRpc.api.BuyVarietyStore)
	gameRpc.rpcHandlers["buyCheck"] = MakeHandler(gameRpc.api.BuyCheck)

	gameRpc.rpcHandlers["userJoin"] = MakeHandler(gameRpc.api.UserJoin)
	gameRpc.rpcHandlers["userLogin"] = MakeHandler(gameRpc.api.UserLogin)
	gameRpc.rpcHandlers["userSave"] = MakeHandler(gameRpc.api.UserSave)
	gameRpc.rpcHandlers["userLoad"] = MakeHandler(gameRpc.api.UserLoad)
	gameRpc.rpcHandlers["setSubscribe"] = MakeHandler(gameRpc.api.SetSubscribe)

	gameRpc.rpcHandlers["Request"] = MakeHandler(gameRpc.api.Init)
	gameRpc.rpcHandlers["getUpdateTime"] = MakeHandler(gameRpc.api.GetUpdateTime)
	gameRpc.rpcHandlers["defaultSettingList"] = MakeHandler(gameRpc.api.DefaultSettingList)
	//gameRpc.rpcHandlers["getGameDataList"] = MakeHandler(gameRpc.api.GetGameDataList)
	gameRpc.rpcHandlers["getServerTime"] = MakeHandler(gameRpc.api.GetServerTime)

	gameRpc.staticResponses["getEventRewardList"] = ""
	gameRpc.staticResponses["getVarietyStore"] = ""
	gameRpc.staticResponses["getPostTime"] = "QlpoNDFBWSZTWbMdTd4AAB1jmX9cEAhUAQKqzABAAAgAIAAgAFRRMTE0YBP1GgNNJlGmmgAD1BUqJEMUFj2zo8qMyhlJMEBIKC+dasIIfd0hYRTGI+C7SAI2poLe+C7kinChIWY6m7w="
	gameRpc.staticResponses["getGameDataList"] = string(embeds.RawGameData)

	mux.HandleFunc("/{category}/{call}/en/", gameRpc.ServeRPC)

	return gameRpc
}

func getRequestDto(r *http.Request) (RpcRequestDto, error) {
	err := r.ParseForm()
	if err != nil {
		return RpcRequestDto{}, err
	}

	requestDto := FormDataToRpcRequestDto(r.Form)
	return requestDto, nil
}

func FormDataToRpcRequestDto(formData url.Values) RpcRequestDto {
	log.Debug().Any("formData", formData).Send()
	return RpcRequestDto{
		Call:         formData.Get("call"),
		TapsonicData: formData.Get("tapsonic_data"),
		AccessToken:  formData.Get("access_token"),
		CacheControl: formData.Get("cache_control"),
		CurrentTime:  formData.Get("current_time"),
		Debug:        formData.Get("debug") == "true",
	}
}

type RpcRequestDto struct {
	Call         string `json:"call"`
	TapsonicData string `json:"tapsonic_data"`
	AccessToken  string `json:"access_token"`
	CacheControl string `json:"cache_control"`
	CurrentTime  string `json:"current_time"`
	Debug        bool   `json:"debug"`
}

func InternalErrorHandler(w http.ResponseWriter, err error) {
	log.Error().Err(err).Msg("[Game Server]: Internal server error")
	http.Error(w, "Internal server error", http.StatusInternalServerError)
}
